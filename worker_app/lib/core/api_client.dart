...
class ApiClient {
  ...
  Future<dynamic> _multipart(
    ...
    final streamed = await request.send().timeout(requestTimeout ?? timeout);
  ...
  Future<(String token, WorkerUser user)> login(
    String username,
    String password,
  ) async {
    final json =
        await _postJson('/auth/login', {
              'username': username,
              'password': password,
            })
  ...
  Future<List<Alert>> myAlerts({int limit = 100}) async {
    final json =
        await _get('/me/alerts', {'limit': '$limit'}) as Map<String, dynamic>;
  ...
  Future<ZoneOccupancy?> myZone() async {
    ...
    return occ == null
        ? null
        : ZoneOccupancy.fromJson(occ as Map<String, dynamic>);
  }
  ...
  Future<(double, String?)> enterZone(String zoneId) async {
    final json =
        await _postEmpty('/zones/$zoneId/enter') as Map<String, dynamic>;
    final closed = json['closed_previous'] as Map<String, dynamic>?;
    return (
      (json['entered_at'] as num?)?.toDouble() ?? 0,
      closed?['zone_id'] as String?,
    );
  }

  Future<double?> leaveZone(String zoneId) async {
    final json =
        await _postEmpty('/zones/$zoneId/leave') as Map<String, dynamic>;
  ...
  Future<List<WorkerQuestion>> myQuestions({int limit = 100}) async {
    final json =
        await _get('/questions', {'limit': '$limit'}) as Map<String, dynamic>;
    return (json['questions'] as List)
        .map((e) => WorkerQuestion.fromJson(e as Map<String, dynamic>))
        .toList();
  }

  Future<WorkerQuestion> question(String id) async => WorkerQuestion.fromJson(
    await _get('/questions/$id') as Map<String, dynamic>,
  );

  // -- worker-invoked assistant --------------------------------------------
  ...
  Future<AssistantReply> assistant({
    ...
    final json = await _multipart(
      ...
      requestTimeout: const Duration(seconds: 50),
    );
    return AssistantReply.fromJson(json as Map<String, dynamic>);
  }

  Future<MeasurementResult> measure({
    required List<List<double>> referencePoints,
    required List<List<double>> measurementPoints,
    required double referenceMm,
    double? specMm,
    double toleranceMm = 5,
    required double imageWidth,
    required double imageHeight,
  }) async {
    final json = await _postJson('/assistant/measure', {
      'reference_points': referencePoints,
      'measurement_points': measurementPoints,
      'reference_mm': referenceMm,
      'spec_mm': specMm,
      'tolerance_mm': toleranceMm,
      'image_size': [imageWidth, imageHeight],
    });
    return MeasurementResult.fromJson(json as Map<String, dynamic>);
  }
  ...
  Future<void> markMessagesRead(String workerId) =>
      _postEmpty('/messages/$workerId/read');
